...
struct UpdateAchievementsTool: ContentTool {
    ...
    static let description =
        ...
        + "predicates over the graded signals (" + MCPAchievementSignalProse.commaList + ") "
        + "combined with `match` (all/any); " + MCPAchievementSignalProse.unitsClause + ". testRefs "
        + "and sectionRefs are validated against the suite; ids must be unique. A classWide goal "
        + "accepts at most one condition and it must be 'grade atLeast X', "
        + "'itemsCovered atLeast N' or 'classCoverage atLeast P' (the class sweep evaluates only "
        + "those shapes). For an itemsCovered or classCoverage goal, classPercent is the share of "
        + "the class that must have contributed — at least one covered item, or at least one cell "
        + "in the class corpus — and the goal is met only when that breadth and the count (or the "
        + "coverage percent) are both reached. Use "
