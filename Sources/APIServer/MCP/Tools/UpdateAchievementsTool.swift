...
struct UpdateAchievementsTool: ContentTool {
    ...
    static let description =
        ...
        + "accepts at most one condition and it must be 'grade atLeast X' or "
        + "'itemsCovered atLeast N' (the class sweep evaluates only those shapes). For an "
        + "itemsCovered goal, classPercent is the share of the class that must have contributed at "
        + "least one covered item — the goal is met when both that breadth and the item count are "
        + "reached. Use "
