...
private func applyNotebookSubstitutionsIfNeeded(
    ...
    // Slice 2: evaluate per-student expressions if any are declared.
    // Pull the student's seed (lazy-generated by AssignmentSeedStore)
    // and feed it + every static var into the evaluator subprocess.
    if !manifest.globalExpressions.isEmpty {
        do {
