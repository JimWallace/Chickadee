...
//
// Instructor authoring for the unified Achievements editor table.
//
// GET returns one editable row per achievement of any kind; PUT replaces the
// manifest's `achievements` array wholesale.  `AchievementInput` is a flat,
// editor-friendly projection of `Achievement`: the kind-specific parameters
// appear as optional fields.  Achievements are server-evaluated and
// display-only — they never change what the suite grades — so, unlike suite
// edits, this path does NOT retest submissions, reschedule validation, or
