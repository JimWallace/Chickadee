...
struct EditableSuiteRow: Encodable {
    ...
    let generatedByName: String? // human-readable family name, nil for hand-written scripts
    ...
    /// "1" when this row was generated by a pattern family, "0" otherwise.
    /// Leaf has no truthiness for strings, so we carry it as a string flag.
    var isGeneratedFlag: String { generatedBy != nil ? "1" : "0" }

    /// Family provenance string for the badge text: "From <name>" or empty.
    var generatedByDisplay: String {
        guard let fid = generatedBy else { return "" }
        return generatedByName ?? fid
    }
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case dependsOnJSON
        case isGeneratedFlag
        case generatedByDisplay
    }
    ...
    func encode(to encoder: Encoder) throws {
        ...
        try container.encodeIfPresent(generatedByName, forKey: .generatedByName)
        ...
        try container.encode(isGeneratedFlag, forKey: .isGeneratedFlag)
        try container.encode(generatedByDisplay, forKey: .generatedByDisplay)
    }
}
