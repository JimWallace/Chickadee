...
/// One-time boot sweep removing pre-v0.4 notebook artifacts (flat
/// `assignment.ipynb` / `submission.ipynb` / `sub_*.ipynb` files directly
/// under `users/{slug}/`, predating the per-setup subdirectory layout).
/// Current working copies live in `users/{slug}/{setupID}/` and are never
/// touched.  Best-effort: unreadable directories are skipped silently.
///
/// Historically this ran on every notebook page view; since the artifacts
/// are static leftovers, it now runs once at startup via
/// `LegacyNotebookCleanupLifecycleHandler`.
func removeLegacyUserNotebookCopies(publicDirectory: String, logger: Logger) {
    let roots = [
        publicDirectory + "files/",
        publicDirectory + "jupyterlite/files/",
        publicDirectory + "jupyterlite/lab/files/",
        publicDirectory + "jupyterlite/notebooks/files/",
    ]

    let fileManager = FileManager.default
    var removedCount = 0
    for root in roots {
        let usersDir = root + "users/"
        guard let userSlugs = try? fileManager.contentsOfDirectory(atPath: usersDir) else { continue }

        for slug in userSlugs {
            let userDir = usersDir + slug + "/"
    ...
    if removedCount > 0 {
        logger.info("legacy_notebook_copies_removed count=\(removedCount)")
    }
}

/// Runs the legacy notebook-copy sweep once at startup (best-effort).
struct LegacyNotebookCleanupLifecycleHandler: LifecycleHandler {
    func didBoot(_ application: Application) throws {
        removeLegacyUserNotebookCopies(
