...
public enum TestStatus: String, Sendable, CaseIterable {
...
// Codable is unavailable in Embedded Swift (no reflection-based coding). Only
// the native targets (Core, worker, server) serialise TestStatus, and they are
// never built embedded — so conform there only, keeping RunnerCore's embedded
// (browser wasm) build clean.
#if !hasFeature(Embedded)
extension TestStatus: Codable {}
#endif
