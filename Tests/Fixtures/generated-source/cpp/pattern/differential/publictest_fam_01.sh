...
// Edit the family, not this file. spec_hash: 869b02122b744a56
...
int ck_ref_classify(double x) { return 1; }
int main() {
    auto x = 18.49;
    try {
        decltype(ck_ref_classify(x)) expected{};
        try {
            expected = ck_ref_classify(x);
        } catch (const std::exception& ck_ref_e) {
            ck::errored(std::string("the reference implementation raised\n")
                + "  input:    " + "x=" + ck::format(x) + "\n"
                + "  error:    " + ck_ref_e.what());
        }
