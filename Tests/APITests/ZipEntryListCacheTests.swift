...
@Suite final class ZipEntryListCacheTests {
    ...
    private func writeZip(name: String, entries: [String: String]) async throws -> String {
        ...
        try await repackZipFromDirectory(zipPath: zipPath, sourceDir: sourceDir)
        return zipPath
    }

    @Test func entriesListsZipContentsAndCaches() async throws {
        let cache = ZipEntryListCache()
        let zipPath = try await writeZip(
            name: "entries",
    ...
    @Test func zipWithoutNotebookReportsFalse() async throws {
        let cache = ZipEntryListCache()
        let zipPath = try await writeZip(name: "no-notebook", entries: ["test_a.sh": "exit 0"])
    ...
    @Test func rewritingZipInvalidatesCachedEntries() async throws {
        ...
        // returned.
        let replacement = try await writeZip(name: "mutating-replacement", entries: ["test_a.sh": "exit 0"])
