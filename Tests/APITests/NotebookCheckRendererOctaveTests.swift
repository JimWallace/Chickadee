...
@Suite(.serialized, .timeLimit(.minutes(3))) struct OctaveNotebookCheckExecutionTests {
    ...
    private func run(_ check: NotebookCheck, submission: String) async throws -> Int32 {
        let generated = renderNotebookCheck(check, language: .octave)
        return try await OctavePatternFamilyExecutionTests.run(
    ...
    @Test(.requiresOctave) func variableExistsPassesAndFails() async throws {
        ...
        #expect(try await run(typed, submission: "total = 10;\n") == 0)
    ...
    @Test(.requiresOctave) func functionExistsHonoursArity() async throws {
        ...
        #expect(try await run(bare, submission: "x = 1;\n") == 2)
    ...
    @Test(.requiresOctave) func numericArrayClosePassesAndFails() async throws {
        ...
        #expect(try await run(close, submission: "results = [1.0; 2.5; 4.0];\n") == 0)
        #expect(try await run(close, submission: "results = [1.0, 2.5, 9.0];\n") == 1)
    ...
    @Test(.requiresOctave) func cellContainsMatchesLiterallyAndByRegex() async throws {
        ...
        #expect(try await run(pcre, submission: "for k = 1:5\n  disp(k);\nend\n") == 0)
    ...
    @Test(.requiresOctave) func figureCountCountsHeadlessFigures() async throws {
        ...
        let plotting = """
            ...
            plot(3:-1:1);
            """
        let result = try await run(figures, submission: plotting)
        ...
        #expect(
            ...
            """)
        #expect(try await run(figures, submission: "x = 1:3;\n") == 1)
    }
