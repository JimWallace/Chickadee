...
//
// Shared base class for the WebRoutes test suite.  Subclasses focus on
// one route group (Index, SubmitHistory, SubmissionPage, Badges) so
...
class WebRoutesTestCase: XCTestCase {
    ...
    func studentUser() async throws -> APIUser {
        try await APIUser.query(on: app.db).filter(\.$username == "student1").first()!
    }

    func makeCourse() async throws -> APICourse {
    ...
    func enrollUser(_ user: APIUser) async throws {
    ...
    func submitMultipartBody(boundary: String, csrfToken: String) -> ByteBuffer {
