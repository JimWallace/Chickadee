...
//
// Shared base class for the AssignmentRoutes test suite.  Each concrete
// subclass focuses on one route group (Dashboard, Publish, Lifecycle,
// Retest, NotebookReset) so XCTest's --parallel scheduler can spread
// them across workers.
...
class AssignmentRoutesTestCase: XCTestCase {
    ...
    func enrollStudentInTestCourse(_ student: APIUser) async throws {
        let courseID = try await app.testCourseID(enrollmentMode: .auto)
    ...
    func makeZip(at path: String, entries: [(String, String)]) throws {
