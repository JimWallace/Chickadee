...
@Suite struct RuntimeSourceDriftTests {
    ...
    @Test func testRuntimeCppMatchesCanonical() throws {
        let canon = try canonical("Tools/runner-support/test_runtime.hpp")
        #expect(
            normalizedCode(testRuntimeCpp, comment: "//") == normalizedCode(canon, comment: "//"),
            "`testRuntimeCpp` has drifted from Tools/runner-support/test_runtime.hpp. Re-sync.")
    }
