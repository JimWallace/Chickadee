...
- `docs/ci-flakiness.md` — CI flake families, evidence, and attack order (started 2026-07, extended through 2026-09-16; start here before chasing a red check on an unrelated PR). Five families; the two newest are open. Family 5 is the one that most often gets misread as your diff: `api-tests` reporting `cancelled` at its 25-minute ceiling, which looks identical to a wedge but is a throughput collapse — the tells are whether tests were still *completing* at the tail of the log, whether `api-tests-postgres` (same target, same commit) passed, and what the `[ci-pressure]` telemetry lines say. Do NOT read anything into per-test or per-suite durations in that log: Swift Testing starts a test's clock when it is scheduled rather than when it gets a parallelization slot, so a healthy run's median test already reports ~80 s and every suite always ends at about the run's total. That entry was rewritten on 2026-09-16 against 213 `main` runs; it now records what is established (it is not fleet-wide runner slowness — the CPU-bound `build` lane has zero excursions in the same population — and the lane was I/O-stall-bound, plus re-running 60 migrations against ~1,100 per-test applications) and what is still open (why throughput collapses 3-5x on ~11 % of runs). Three merged changes made BOTH lanes O(1) in the migration count, which is the property that matters as the suite grows — a tmpfs `/tmp`, SQLite test databases copied from a once-migrated template, and postgres test schemas recycled from a pre-migrated pool (`api-tests` 291 s → 143 s on `main`; `api-tests-postgres` 391 s → 119 s measured locally). That roughly doubles the collapse the lane can absorb, so **the symptom may stop appearing without the cause having been found** — the acceptance test is the `main` population, not a green run
